...
# 💡【あなたの神アイデア】店名の先頭3文字以上が一致するかチェック
...
if st.button("🐾 あなたのアイデア（店名＋日付）で完璧に仕分け！", use_container_width=True, type="primary"):
    if not pdf_files:
                    ...
                    try:
                        ...
                        with pdfplumber.open(pdf_bytes) as pdf_text:
                            for page_num in range(len(reader.pages)):
                                ...
                                # 💡【究極の明示的ルール】3つのうち、強力な組み合わせを満たせば合格！
                                for item in statements:
                                    if not item['matched']:
                                        ...
                                        if amt_match and date_match and shop_match:
                                            is_valid, reason, priority = True, "金額＋日付＋店名(完璧)", 3
                                        elif amt_match and shop_match:
                                            is_valid, reason, priority = True, "金額＋店名", 2
                                        elif shop_match and date_match: # 💡 あなたのアイデア！金額が読めなくても救済！
                                            is_valid, reason, priority = True, "店名＋日付(金額読取不可を補完)", 2
