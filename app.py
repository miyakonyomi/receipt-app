...
st.markdown("仕分けエラー率0%！「金額・日付・店名」の三柱照合と、誤検知防止フィルターで完璧に仕分けます🐾")
st.divider()
...
if st.button("🐾 トライアングル照合（ハイジャック防止版）で仕分け！", use_container_width=True, type="primary"):
    if not pdf_files:
        st.warning("⚠️ レシートPDFがアップロードされていません。")
    else:
        with st.spinner('🐻 3612円の奪還作戦を実行中...'):
